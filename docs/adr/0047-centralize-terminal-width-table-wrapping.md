...
Add exact dependency `terminal_size = "=0.4.4"` and centralize final table rendering in
`output::formatting`. The renderer resolves a positive `BZR_TABLE_WIDTH` value first. Otherwise it
queries the stdout handle with `terminal_size_of`; unsupported platforms or failed queries produce
no width. A resolved width applies `tabled::settings::Width::wrap` to the whole table before it is
written. No resolved width preserves today's rendering byte-for-byte.
...
automated tests can request the same layout without a PTY. Invalid, zero, or out-of-range values are
ignored with a warning before normal stdout detection. At widths below the grid's irreducible
borders and padding, `tabled` emits the smallest structurally valid grid rather than removing
columns or corrupting boundaries.
...
- Very narrow widths may still exceed the requested width when the selected column count alone
  requires more border and padding cells than the terminal provides.
...
- **Thread width metadata through `Writers`, every command, and every resource writer.** judgment:
  this expands signatures across the command layer even though width affects only the final grid.
