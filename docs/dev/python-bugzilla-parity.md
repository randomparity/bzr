...
| RHBZ ExternalBugs add | `bzr bug external-bug add` | RHBZ-only XML-RPC capability, real-server readback | `compare/08-rhbz-externalbugs/add` |
| RHBZ ExternalBugs update | `bzr bug external-bug update` | RHBZ-only XML-RPC capability, real-server readback | `compare/08-rhbz-externalbugs/update` |
| RHBZ ExternalBugs remove | `bzr bug external-bug remove` | RHBZ-only XML-RPC capability, real-server readback | `compare/08-rhbz-externalbugs/remove` |
...
| `--product`, `--version`, `--component`, `--summary`, `--short_desc`, `--comment`, `--long_desc`, `--sub-component`, `--os`, `--arch`, `--severity`, `--priority`, `--alias`, `--status`, `--bug_status`, `--url`, `--target_milestone`, `--target_release`, `--blocked`, `--dependson`, `--keywords`, `--groups`, `--cc`, `--assigned_to`, `--assignee`, `--qa_contact`, `--private`, `--id`, `--bug_id`, `--reporter` | parity / missing reads | Core create, query, update, and view fields are parity. RHBZ sub-components and target release are proven writable and readable through `bzr bug view`; Python's query-only field spellings remain unavailable as bzr search filters. | `compare/01-bug-lifecycle/create`, `compare/01-bug-lifecycle/query`, `compare/01-bug-lifecycle/update`, `compare/09-rhbz-fields/sub-components`, `compare/09-rhbz-fields/target-release` |
| `--quicksearch`, `--savedsearch`, `--savedsearch-sharer-id`, `--from-url`, `--emailtype`, `--components_file`, `--url_type`, `--keywords_type`, `--status_whiteboard_type`, `--fixed_in_type` | parity / explicit gaps / non-goals | Quicksearch, URL import, and URL/whiteboard/email match types are parity. Saved search is #670; `--components_file`, keyword match types, and RHBZ fixed-in query matching have no bzr counterpart and are non-goals. | `compare/01-bug-lifecycle/query`, `compare/01-bug-lifecycle/saved-search`, `compare/01-bug-lifecycle/query-match-types`, `compare/09-rhbz-fields/fixed-in` |
...
| `getbugfields`, `product_get`, `refresh_products`, `getproducts`, `getcomponentsdetails`, `getcomponentdetails`, `getcomponents`, `addcomponent`, `editcomponent`, `getbug`, `getbugs`, `get_comments`, `build_query`, `query_return_extra`, `query`, `pre_translation`, `post_translation`, `bugs_history_raw` | parity / non-goals / explicit gap | Corresponding resource commands are parity; cache refresh, query construction, and translation hooks are Python-library internals with no bzr library surface. RHBZ component update is the open #802 gap. | `compare/00-products/list-products`, `compare/01-bug-lifecycle/query`, `compare/01-bug-lifecycle/history`, `compare/05-products-components/component-create`, `compare/08-rhbz-externalbugs/component-update` |
...
| `add_external_tracker`, `update_external_tracker`, `remove_external_tracker` | `bug external-bug add`, `update`, `remove` | RHBZ ExternalBugs mutations use the advertised extension over XML-RPC. | `compare/08-rhbz-externalbugs/add`, `compare/08-rhbz-externalbugs/update`, `compare/08-rhbz-externalbugs/remove` |
