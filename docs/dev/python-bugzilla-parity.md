...
| `--login`, `--username`, `--password`, `--restrict-login`, `--ensure-logged-in`, `--no-cache-credentials`, `--cookiefile`, `--tokenfile`, `--api-key`, `pos_username`, `pos_password` | explicit gaps / non-goals | Login, logout, and restricted login are #681; token transport is #676. Cookie files, interactive password prompts, positional credentials, and python-bugzilla cache toggles are non-goals because bzr uses explicit API-key/token credential sources and never imports browser cookies. | `compare/06-auth-config-tls/login-command-gap`, `compare/06-auth-config-tls/token-transport-gap`, `compare/06-auth-config-tls/restricted-login`, `compare/06-auth-config-tls/cached-token` |
...
| `--close`, `--dupeid`, `--reset-assignee`, `--reset-qa-contact`, `--file`, `--description`, `--type`, `--get`, `--getall`, `--get-all`, `--ignore-obsolete`, `--comment-tag`, `--minor-update`, `--flag`, `--tags`, `--whiteboard`, `--status_whiteboard`, `--devel_whiteboard`, `--internal_whiteboard`, `--qa_whiteboard`, `--fixed_in` | parity / explicit gaps / non-goals | Close/duplicate, attachment transfer, comments, flags, comment tags, minor update, and obsolete filtering are parity. Tags are #680. RHBZ whiteboards and fixed-in are #775; reset-assignee/reset-qa-contact are non-goal convenience aliases. | `compare/01-bug-lifecycle/update-options`, `compare/03-attachments/upload-metadata-comment`, `compare/03-attachments/ignore-obsolete`, `compare/01-bug-lifecycle/bug-tags`, `compare/09-rhbz-fields/whiteboards`, `compare/09-rhbz-fields/fixed-in` |
| `url_to_query`, `fix_url`, `get_rcfile_default_url`, `bz_ver_major`, `bz_ver_minor`, `readconfig`, `connect`, `is_xmlrpc`, `is_rest`, `get_requests_session`, `disconnect` | non-goal | Python library connection/configuration helpers have no public bzr library API; bugzillarc import remains gap #682. | `compare/06-auth-config-tls/bugzillarc-import-gap` |
| `login`, `interactive_save_api_key`, `interactive_login`, `logout`, `logged_in` | explicit gaps / non-goals | `login` and `logout` are #681; interactive credential prompts and library-session inspection are non-goals for a CLI with explicit credential sources. | `compare/06-auth-config-tls/login-command-gap`, `compare/06-auth-config-tls/logout` |
