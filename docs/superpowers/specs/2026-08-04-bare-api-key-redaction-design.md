...
controls configuration and is trusted with the key. Logs, transcripts, and copied error
reports are treated as destinations that must not receive that key.
