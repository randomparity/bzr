...
| `--no-color` | Disable colored output on stdout and on the stderr diagnostic stream. Color is also suppressed automatically per stream, each following its own TTY status. |
...
> **Note:** Resolving a server-side saved search is a Red Hat Bugzilla extension. bzr checks the server's advertised extension list over the API mode in use before searching, and exits 15 when the server does not advertise it, rather than returning an unfiltered result — a stock Bugzilla accepts `savedsearch`/`sharer_id` and silently ignores them. Confirmed against Bugzilla 5.0.6, 5.2 and 5.3.3+: none implements the parameters. If the check itself cannot be completed, bzr says so distinctly instead of claiming the extension is absent; the structured error carries `capability_status` (`absent` or `undetermined`) so a script can tell the two apart. The check applies to `--saved-search`; parameters passed through `--from-url` (including a `buglist.cgi` saved-search URL) are forwarded verbatim and are **not** gated. `bzr server info` still reads the server version over REST, so it can fail on a REST-disabled deployment where this check now succeeds.
