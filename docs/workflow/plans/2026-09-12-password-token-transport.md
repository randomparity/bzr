...
Expected implementation size: 180–250 changed lines (M) — derived from the
...
- `src/client/mod.rs`, `src/client/transport_tests.rs`: prepared REST token
  auth and XML-RPC/hybrid rejection.
...
4. Commit the focused model, resolver, and redaction contract.
...
auth; token configurations permit `ApiMode::Rest` only.
...
- Mode: focused-test. Contract: token connections neither probe nor persist
  API-key auth-method state, and XML-RPC/hybrid selection fails before wire
  dispatch. Red: add connection tests first; green:
...
3. Add the REST-only validation/error path for token-backed clients.
4. Commit the connection and transport contract.

## Task 3 — Prove and document parity

**Interfaces:** the existing auth comparison phase configures the token source
without new CLI flags and verifies a REST identity request; the parity row
changes from expected gap to parity.
...
- Mode: functional-test. Contract: a token from config authenticates a real
  ...
  gap. Command: `make functional-test`; expected: all phases pass.
...
1. Replace only #676's controlled-gap assertion with a configured-token REST
   assertion in the existing auth comparison phase.
