...
**Architecture:** `output::formatting` resolves width and owns the only completed-table write path.
Generic resource tables already enter through `write_table_records`; bespoke bug grids will call the
same finalizer. The design is governed by
...
Expected implementation size: 190–290 changed lines (M) — derived from one shared resolver/finalizer,
focused formatter and bug-resource tests, one functional case, dependency metadata, and CLI docs.
...
- Modify `src/output/formatting.rs`: width parsing/detection and shared table finalization.
...
**Files:** `Cargo.toml`, `Cargo.lock`, `src/output/formatting.rs`,
`src/output/formatting_tests.rs`.

**Interfaces**

- Consume existing `tabled::builder::Builder::build() -> Table` and
  `tabled::settings::Width::wrap(usize)` from tabled 0.21.0.
- Add `fn resolve_table_width(explicit: Option<&str>, detected: Option<u16>) -> Option<usize>`.
...
- Add `pub(super) fn write_table<W: Write + ?Sized>(table: Table, out: &mut W)` for production.
- Add a private `write_table_with_width<W: Write + ?Sized>(table: Table, width: Option<usize>,
  out: &mut W)` used by production and formatter tests.
- `write_table_records` continues to expose its existing signature and delegates its completed table
  to `write_table`.

**Verification**

- Mode: focused-test — resolution precedence and invalid fallback in new
  `table_width_*` cases; before implementation they fail to compile because the resolver is absent;
  green command: `make test-one T=table_width_` with all matching tests passing.
...
2. Add injected finalizer tests that build a long ASCII/Unicode table and assert the output line
   widths; run `make test-one T=write_table_` and observe unresolved function failures.
3. Exact-pin `terminal_size = "=0.4.4"` in `Cargo.toml` and run `cargo update -p terminal_size
   --precise 0.4.4`; expect `Cargo.lock` to contain terminal_size 0.4.4 and its platform dependencies.
4. Implement pure resolution, cfg-gated stdout detection, invalid-value warning, and the injected
   `Width::wrap` finalizer. Make `write_table_records` delegate to it.
5. Run the three focused commands from Verification and expect all matching tests to pass.
6. Commit as `feat(output): wrap shared tables to terminal width` after `cargo fmt -- --check` and
...
- Consume `output::formatting::write_table(Table, &mut W)` from Task 1.
- Preserve `write_bugs`, `write_bug_links`, and `write_bug_adjacency` public signatures.
...
- Mode: focused-test — list, links, requests, and canonical adjacency grids share the finalizer;
  new `bug_table_width_*` tests set an injected explicit width through the formatter test seam or a
  serialized `BZR_TABLE_WIDTH` environment scope, and fail red because current direct writes exceed
  it; green command: `make test-one T=bug_table_width_`.
...
1. Add serialized width tests for all four completed grids and one JSON-family regression; run the
   named focused commands and observe the table-width cases fail on oversized lines.
2. Import `write_table`, replace each direct `writeln!(..., builder.build())`, and leave record
   construction unchanged.
3. Run `make test-one T=bug_table_width_`, the two JSON-family commands, and the existing adjacency
   and list table tests; expect all to pass with preserved headers and rows.
4. Commit as `refactor(output): share wrapping across bug tables` after
