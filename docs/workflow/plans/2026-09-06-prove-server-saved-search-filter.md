...
Expected implementation size: 300–380 changed lines (M) — derived from the file map below:
~180 in the proxy including its `ShapeTests`, ~60 in the lifecycle phase, ~15 in
`run-compare.sh`, ~60 in the harness self-test, and 2 record lines.
...
| `tests/functional/pybz/container-tests.sh` | modify | stubs for the proxy lifecycle and the proxied call; two new failure controls; count and fixture-row updates |
...
Expect `OK` and a test count six higher than before the task.

**Acceptance criteria.** All six cases pass; the existing `ShapeTests` still pass; with
`BZR_FUNC_REDHAT_MODE` unset or set to another mode, no `/rest/bug` response is filtered.
...
- Contract: the seeder accepts one or more ids and rejects zero ids or a non-decimal id.
  Mode: focused-test. Test: the argument-validation branch is exercised by Task 4's stub
  parity check (`container-tests.sh`), which asserts the stub and the real signature agree
  on arity. Expected red: the stub's `[[ $# -ge 3 ]]` guard fails against a 4-argument
  call site that was never updated. Green:
  `make functional-compare` reaching `compare/01-bug-lifecycle/saved-search`.
...
**Interfaces produced.** `lifecycle_ids_differ <file_a> <file_b>`,
`lifecycle_ids_contain <file> <json_id_array>`, `lifecycle_saved_search_probe <name> <args...>`,
and the `LIFECYCLE_BZR_URL` override read by `lifecycle_bzr_probe`. Task 4 stubs
`redhat_shape_start` / `redhat_shape_stop` and keys its controls on the capture-file names
`saved-search-control` and `saved-search-filtered`.

### Verification

- Contract: the filtered id set differs from the unfiltered control, and equals the seeded
  subset. Mode: focused-test. Test: `run_lifecycle_failure_control` with
  `LIFECYCLE_SAVED_SEARCH_UNFILTERED` in Task 4 — the proxied call returns the control set
  and the row must go red. Expected red before this task: the control does not exist, so
  the flag has no effect and the row still passes. Green:
...
**Step 3.2.** Add the two id helpers immediately after `lifecycle_ids_are`:

```bash
# `lifecycle_ids_are` is exact-equality. These two cover the other shapes the
# saved-search row needs: that two id sets are not the same set, and that a set
# contains ids a filtered result would have excluded.
lifecycle_ids_differ() {
    local first="$1" second="$2"

    if ! jq -es 'length == 2 and all(.[]; type == "array")' \
        "$first" "$second" >/dev/null; then
        test_fail "ID evidence for the filtered comparison had an invalid structure"
        return 1
    fi
    if jq -es '(.[0] | map(.id) | sort) == (.[1] | map(.id) | sort)' \
        "$first" "$second" >/dev/null; then
        return 1
    fi
    return 0
...
if [[ -n $LIFECYCLE_BZR_ID && -n $LIFECYCLE_PYBZ_ID ]] &&
    ...
    lifecycle_ids_are "$COMPARE_EXCHANGE_DIR/saved-search-control.bzr.stdout.json" \
    ...
    lifecycle_ids_are "$COMPARE_EXCHANGE_DIR/saved-search-filtered.bzr.stdout.json" \
        "[$LIFECYCLE_BZR_ID]" &&
    lifecycle_ids_differ "$COMPARE_EXCHANGE_DIR/saved-search-filtered.bzr.stdout.json" \
        "$COMPARE_EXCHANGE_DIR/saved-search-control.bzr.stdout.json" &&
...
Three things about this shape are deliberate and must not be rearranged:
...
`saved-search-filtered.bzr.stdout.json` holds one id and
`saved-search-control.bzr.stdout.json` holds two.
...
**Interfaces produced.** Controls `LIFECYCLE_SAVED_SEARCH_UNFILTERED` and
...
**Step 4.1.** Update the `seed_server_saved_search` stub for the new arity:
...
**Step 4.6.** Add the two controls. `LIFECYCLE_SAVED_SEARCH_UNFILTERED` and
`LIFECYCLE_SAVED_SEARCH_PYBZ_FILTERED` both fail in the **precondition** chain, so the row
never reaches `lifecycle_expect_gap` and the outcome is an outright FAIL — which is
`run_lifecycle_failure_control`, not `run_gap_ineligible_control`. Add beside the other
`run_lifecycle_failure_control` calls:

```bash
    if ! run_lifecycle_failure_control LIFECYCLE_SAVED_SEARCH_UNFILTERED saved-search \
        'server saved search'; then
        control_failures=$((control_failures + 1))
    fi
    if ! run_lifecycle_failure_control LIFECYCLE_SAVED_SEARCH_PYBZ_FILTERED saved-search \
        'server saved search'; then
        control_failures=$((control_failures + 1))
    fi
```
...
`3 "$GAP_COUNT"` to hold, and two new `controlled red: saved-search ...` lines.
...
**Acceptance criteria.** Both new controls report `controlled red`; the default-scenario
counts are unchanged at 7/0/3; the stale-gap scenario still reports `#670 appears
resolved` and `assert_equals 3 "$FAIL_COUNT" "stale gap fail count"` holds.

## Task 5 — the parity record and its fixture copy

Modifies `docs/dev/python-bugzilla-parity.md` and
`tests/functional/pybz/container-tests.sh`. These two strings must stay byte-identical and
nothing enforces it, so both edits are in one task.
...
**Step 5.3.** Verify the two agree mechanically rather than by eye:

```bash
diff <(rg -N '^\| Server saved search \|' docs/dev/python-bugzilla-parity.md) \
     <(rg -No "\| Server saved search \|[^']*" tests/functional/pybz/container-tests.sh)
```

Expect no output.

**Acceptance criteria.** `run_parity_report_fixture` passes; the diff above is empty.
...
**Step 6.3.** Confirm the discrimination held on real data rather than assuming it: for one
version, confirm the two captured id sets actually differed. The exchange directory is
removed on exit, so assert it inside the run instead — `lifecycle_ids_differ` already does
exactly this, and its presence in the precondition chain is what makes a green row
evidence. Record in the completion report that the row was green **and** that
`lifecycle_ids_differ` was in the chain that produced it.
...
None recorded at plan time. Any `$trial-loop` deferral from the design or branch review is
