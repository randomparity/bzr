...
- Component help, generated completion, the README, the canonical embedded
  `bzr-reference` skill and command metadata, and the embedded dry-run skill expose
  only `list`, `view`, and `create` as component operations.
...
Documentation, the embedded agent references, and functional phases then assert the
smaller public surface rather than describing or probing the deleted path. Current
published sources include `README.md`, `content/skills/bzr-reference/SKILL.md`, both
command-reference files, and `content/skills/bzr-dry-run-confirm/SKILL.md`.
