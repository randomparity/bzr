...
ROOT_GLOBALS='--server --server-url --server-api-key-env --server-email --server-tls-insecure --server-tls-ca-cert --server-tls-pin-sha256 --server-tls-pin-now --output --json --config --no-color --quiet --api --timeout --retry --progress --dry-run --yes'
