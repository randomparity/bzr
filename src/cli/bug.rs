...
#[expect(
    clippy::doc_markdown,
    reason = "doc references literal API names like Bug.get; backticking them would degrade --help readability"
)]
pub struct ViewArgs {
...
#[expect(
    clippy::struct_excessive_bools,
    reason = "each bool is a distinct CLI flag (--no-comment, --add-depends-on, --add-blocks, --no-cc, --no-keywords); they are not a state enum"
)]
