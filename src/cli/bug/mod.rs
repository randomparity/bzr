...
pub(crate) struct CreateFieldArgs {
    ...
    /// empty string. Every key is checked against the server's own field
    /// catalogue before anything is sent, so a name the server does not
    ...
    /// a silent winner. Use `bzr field list` to see what a server declares.
