...
pub(crate) async fn read_body_within(
    ...
    // `from_utf8` reuses the buffer on the valid-UTF-8 path, where
    // `Response::text()` pays a full copy. Invalid UTF-8 costs what `text()`
    // costs — the failed `String` holds the buffer while the lossy decode
    // builds up to three bytes per input byte — which is why ADR 0068 states
    // the peak per path rather than as one multiple.
