...
//! Integration-level flow tests for `bug update`: the full dispatch path
//! through [`crate::commands::bug::execute`], including `--from-json` input,
//! batching, table/JSON output, the `--expect-unchanged-since` guard, and
//! dry-run previews. Direct unit tests for the individual leaf modules live in
//! their own siblings (`draft_tests.rs`, `payload_tests.rs`,
//! `validate_tests.rs`, `execute_tests.rs`, `output_tests.rs`).
...
use super::test_helpers::{
    forbid_put, make_empty_update_action, make_update_action, make_update_action_with_comment,
    make_update_action_with_dupe_of, make_update_action_with_scalar_parity_fields,
    mock_get_bug_lct, mock_put_bug_ok, received_put_count, update_ids_mut,
};
