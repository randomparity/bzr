...
- **`client/`** — `BugzillaClient` wraps reqwest for Bugzilla REST API. Split into per-resource submodules (`bugs.rs`, `attachments.rs`, `comments.rs`, `products.rs`, `users.rs`, `groups.rs`, `components.rs`, `classifications.rs`, `fields.rs`, `server.rs`). `auth.rs` handles auth detection and API mode detection. Public data types live in `types/`.
