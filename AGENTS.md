...
- **`client/`** — `BugzillaClient` wraps reqwest for Bugzilla REST API. Split into per-resource submodules (`bug.rs`, `attachment.rs`, `comment.rs`, `product.rs`, `user.rs`, `group.rs`, `component.rs`, `classification.rs`, `field.rs`, `server.rs`). `auth.rs` handles auth detection and API mode detection. Public data types live in `types/`.
- **`config.rs`** — `Config` and `ServerConfig` structs. TOML file at `~/.config/bzr/config.toml`. Multiple named servers with a default. Re-exports `AuthMethod` and `ApiMode` from `types/common.rs`; `AuthMethod` (`Header`/`QueryParam`) persisted per-server.
...
- **`output/`** — `mod.rs` is a pure re-export facade. `common.rs` holds shared primitives (`print_formatted`, `ActionResult`, `ResourceKind`, field helpers). Per-resource submodules (`bug.rs`, `comment.rs`, `attachment.rs`, `product.rs`, `user.rs`, `group.rs`, `field.rs`, `server.rs`, `config.rs`) each handle one domain type. Uses `tabled` for tables, `colored` for status colors.
- **`commands/`** — Each submodule has an `execute()` function that takes the action enum, optional server name, and output format. All follow the same pattern: load config → resolve auth → connect client → call API → print output. `shared.rs` provides `connect_client()` (config load + auth detection + client construction). `flags.rs` handles Bugzilla flag syntax parsing. Note: the config command lives in `config_cmd.rs` (not `config.rs`) to avoid collision with the config data module. See `src/commands/mod.rs` for the complete list of command modules.
