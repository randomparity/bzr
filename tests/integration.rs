...
async fn product_list_integration() {
    ...
    Mock::given(method("GET"))
        ...
        .respond_with(
            ResponseTemplate::new(200).set_body_json(serde_json::json!({"ids": [1]})),
        )
        .mount(&mock)
        .await;
    Mock::given(method("GET"))
...
async fn component_create_integration() {
    ...
    Mock::given(method("POST"))
        ...
        .respond_with(
            ResponseTemplate::new(200).set_body_json(serde_json::json!({"id": 10})),
        )
        .mount(&mock)
        .await;
...
async fn attachment_list_integration() {
    ...
    assert!(
        result.is_ok(),
        "attachment list should succeed: {result:?}"
    );
}
...
"#,
    ...
    let result = bzr::commands::config_cmd::execute(&action, bzr::output::OutputFormat::Json);
