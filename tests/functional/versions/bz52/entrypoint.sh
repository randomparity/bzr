...
# ── Run checksetup.pl ────────────────────────────────────────────────
# Bugzilla 5.2 checksetup needs 3 passes:
#   1) Generate new localconfig fields (exits early with "Please edit")
#   2) Create schema + admin user
#   3) Finalize
cd "$BZ_DIR"

# Bugzilla 5.2 needs data/ to exist before checksetup can proceed
mkdir -p data

echo "==> Running checksetup.pl (pass 1 — generate localconfig)..."
perl checksetup.pl answers.txt 2>&1 | tail -5 || true

echo "==> Running checksetup.pl (pass 2 — schema)..."
perl checksetup.pl answers.txt 2>&1 | tail -5

echo "==> Running checksetup.pl (pass 3 — finalize)..."
perl checksetup.pl answers.txt 2>&1 | tail -5
