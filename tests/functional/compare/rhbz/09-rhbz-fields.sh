...
RHBZ_FIELDS_BZR_RELEASE="${RHBZ_FIELDS_TOKEN}-bzr-release"
...
RHBZ_FIELDS_BZR_SUB_COMPONENT="${RHBZ_FIELDS_TOKEN}-bzr-sub-component"
RHBZ_FIELDS_FIXED_IN="${RHBZ_FIELDS_TOKEN}-fixed-in"
RHBZ_FIELDS_BZR_FIXED_IN="${RHBZ_FIELDS_TOKEN}-bzr-fixed-in"
...
RHBZ_FIELDS_BZR_DEVEL_WHITEBOARD="${RHBZ_FIELDS_TOKEN}-bzr-devel"
RHBZ_FIELDS_BZR_INTERNAL_WHITEBOARD="${RHBZ_FIELDS_TOKEN}-bzr-internal"
RHBZ_FIELDS_BZR_QA_WHITEBOARD="${RHBZ_FIELDS_TOKEN}-bzr-qa"
...
rhbz_fields_prepare() {
    ...
    # shellcheck disable=SC2034 # Consumed by the following ExternalBugs phase.
...
rhbz_fields_create_bug() {
    ...
    printf '%s\n' \
        ...
        "INSERT INTO bug_rh_sub_components (bug_id, rh_sub_component_id) VALUES (@rhbz_fields_bug_id, $RHBZ_FIELDS_SECOND_SUB_COMPONENT_ID);" \
...
rhbz_fields_probe_bzr() {
    local name="$1" bug_id="$2" field="$3" value="$4" state_filter="$5" expected="$6" fields="$7" bzr_filter="$8"

    if [[ $field == rh_sub_components ]]; then
        local payload
        payload=$(jq -cn --arg component "$RHBZ_FIELDS_COMPONENT" --arg value "$value" \
            '{rh_sub_components:{($component):[$value]}}')
        RUST_LOG=bzr=debug run_bzr --server "$RESOURCE_SERVER" --api rest bug update "$bug_id" \
            --field-json - <<<"$payload"
    elif [[ $name == whiteboards ]]; then
        local devel internal qa
        IFS=, read -r devel internal qa <<<"$value"
        RUST_LOG=bzr=debug run_bzr --server "$RESOURCE_SERVER" --api rest bug update "$bug_id" \
            --field "cf_devel_whiteboard=$devel" --field "cf_internal_whiteboard=$internal" \
            --field "cf_qa_whiteboard=$qa"
    else
        RUST_LOG=bzr=debug run_bzr --server "$RESOURCE_SERVER" --api rest bug update "$bug_id" \
            --field "$field=$value"
    fi
    ...
    if [[ $BZR_EXIT -eq 0 ]] && jq -e . "$BZR_STDOUT" >/dev/null &&
        ...
        rhbz_fields_read "$bug_id" "$fields" "${name}-bzr" &&
        jq -e --arg expected "$expected" "$state_filter" \
            "$COMPARE_EXCHANGE_DIR/rhbz-fields-${name}-bzr-state.json" >/dev/null &&
        rhbz_fields_read_with_bzr "${name}-bzr" "$bug_id" "$fields" "$bzr_filter" "$expected"; then
        test_pass
        return 0
    fi
...
rhbz_fields_run() {
    ...
    if rhbz_fields_prepare && rhbz_fields_metadata_ready &&
        ...
        rhbz_fields_read_with_bzr "$name" "$bug_id" "$pybz_fields" "$bzr_read_filter" \
            "$pybz_expected"; then
        rhbz_fields_probe_bzr "$name" "$bug_id" "$bzr_field" "$bzr_value" "$bzr_filter" "$bzr_expected" "$pybz_fields" "$bzr_read_filter"
...
rhbz_fields_run target-release rhbz_target_release \
    ...
    "$RHBZ_FIELDS_BZR_RELEASE" \
    '.bugs[0].target_release | index($expected)' "$RHBZ_FIELDS_BZR_RELEASE"
...
rhbz_fields_run fixed-in rhbz_fixed_in \
    ...
    "$RHBZ_FIELDS_BZR_FIXED_IN" \
    '.bugs[0].cf_fixed_in == $expected' "$RHBZ_FIELDS_BZR_FIXED_IN"
...
rhbz_fields_run whiteboards rhbz_whiteboards \
    ...
    "$RHBZ_FIELDS_BZR_DEVEL_WHITEBOARD,$RHBZ_FIELDS_BZR_INTERNAL_WHITEBOARD,$RHBZ_FIELDS_BZR_QA_WHITEBOARD" \
    '($expected | split(",")) as $values | .bugs[0].cf_devel_whiteboard == $values[0] and .bugs[0].cf_internal_whiteboard == $values[1] and .bugs[0].cf_qa_whiteboard == $values[2]' \
    "$RHBZ_FIELDS_BZR_DEVEL_WHITEBOARD,$RHBZ_FIELDS_BZR_INTERNAL_WHITEBOARD,$RHBZ_FIELDS_BZR_QA_WHITEBOARD"
