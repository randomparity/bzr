...
if ! declare -F r11_login_control >/dev/null; then
    r11_login_control() {
        local input output="$COMPARE_EXCHANGE_DIR/r11-login.output.json"
        rm -f "$COMPARE_EXCHANGE_DIR/python-bugzilla-token"
        input=$(r11_write_auth_request login) || return 1
        r11_adapter_result_is login "$input" "$output" \
            '.transport == "REST" and .result == {authenticated:true,cache_written:true,restricted:true}'
    }
fi
if ! declare -F r11_cached_control >/dev/null; then
    r11_cached_control() {
        local input output="$COMPARE_EXCHANGE_DIR/r11-cached_auth.output.json"
        input=$(r11_write_auth_request cached_auth) || return 1
        r11_adapter_result_is cached_auth "$input" "$output" \
            '.transport == "REST" and .result == {authenticated:true,cache_used:true}'
    }
fi
if ! declare -F r11_logout_control >/dev/null; then
    r11_logout_control() {
        local input output="$COMPARE_EXCHANGE_DIR/r11-logout.output.json"
        input=$(r11_write_auth_request logout) || return 1
        r11_adapter_result_is logout "$input" "$output" \
            '.transport == "REST" and .result == {cache_cleared:true,logged_out:true}'
    }
fi

if ! declare -F r11_bugzillarc_control >/dev/null; then
    r11_bugzillarc_control() {
...
if ! declare -F r11_tls_control >/dev/null; then
    r11_tls_control() {
...
if ! declare -F r11_certificate_control >/dev/null; then
    r11_certificate_control() {
...
if ! declare -F r11_bearer_control >/dev/null; then
    r11_bearer_control() {
...
if ! declare -F r11_parser_gap >/dev/null; then
    r11_parser_gap() {
        local kind="$1"
        case "$kind" in
        token | bearer)
