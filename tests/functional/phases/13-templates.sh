...
test_begin "bug-view-verify-template-fields" "bug view (verify template fields)"
if [[ -n "$TMPL_BUG" ]]; then
...
test_begin "template-save-metadata-fields" "template save metadata fields"
...
test_begin "template-update-clear-metadata" "template update --clear metadata"
...
test_begin "template-delete-metadata-template" "template delete metadata template"
...
test_begin "template-delete" "template delete"
run_bzr template delete func-tmpl
if assert_success; then test_pass; fi

test_begin "template-show-deleted-expect-failure" "template show (deleted, expect failure)"
run_bzr template show func-tmpl
if assert_failure; then test_pass; fi
